...
function renderMembers() {
  ...
  left.appendChild(presence);
    ...
    // Leave room: show for the current user (non-admins always; admins only if not the last admin)
    if (uid === currentUser?.uid) {
      const leaveBtn = document.createElement('button');
      leaveBtn.className = 'btn-danger';
      leaveBtn.textContent = 'Leave';
      leaveBtn.onclick = () => leaveRoom(adminsCount);
...
async function leaveRoom(adminsCount) {
  membersError.textContent = '';
  try {
    // Guard: admins cannot leave if they are the last admin
    if (myRole === 'admin' && adminsCount <= 1) {
      membersError.textContent = "You are the last admin. Assign another admin before leaving.";
      return;
    }
    const ok = confirm('Are you sure you want to leave this room?');
    if (!ok) return;
    await deleteDoc(doc(db, 'chatrooms', roomId, 'members', currentUser.uid));
    // Redirect back to chat list after leaving
    location.href = 'chatrooms.html';
  } catch (e) {
    membersError.textContent = 'Failed to leave room.';
    console.error(e);
  }
}
