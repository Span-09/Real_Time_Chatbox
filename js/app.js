...
onAuthStateChanged(auth, user => {
  if (user) {
    if (user.isAnonymous) {
      // Do not allow anonymous users into chat
      window.location.href = 'chatrooms.html'
      return
    }
    currentUser = user
  ...
  // Ensure membership gating is evaluated once auth is ready
  checkMembership()
  } else {
    // Require explicit sign-in. Redirect to chat list (shows login modal there).
    window.location.href = 'chatrooms.html'
  }
})

// Determine membership and lock composer for non-members
async function checkMembership() {
  if (!currentUser || !roomId) return
  try {
    const m = await getDoc(doc(db, 'chatrooms', roomId, 'members', currentUser.uid))
    isMemberOfRoom = m.exists()
    toggleComposer(isMemberOfRoom)
  } catch {
...
function toggleComposer(enabled) {
  ...
  messageInput.placeholder = enabled ? 'Type a message...' : 'You are not a member of this room'
  // Clear any lingering error/retry UI when state changes
...
async function sendMessage () {
  ...
  if (!isMemberOfRoom) { showError('You are not a member of this room.', { showRetry: false }); return }
  ...
  try {
    ...
    // Show retry only for transient errors (e.g., permissions/network), not for membership
    const transient = true
    showError('Failed to send.', { showRetry: transient })
...
// Mobile UX tweaks: prevent viewport jump when focusing input
messageInput.addEventListener('focus', () => {
  setTimeout(() => { messageInput.scrollIntoView({ block: 'end' }) }, 250)
})
